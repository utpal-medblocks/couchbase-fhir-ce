...
const FTSIndexTreeDisplay: React.FC<FTSIndexTreeDisplayProps> = React.memo(
  ({ ftsIndexData }) => {
    ...
    const renderField = (
      ...
      if (hasFields) {
        ...
        Object.entries(fieldData.properties!).forEach(
          ([propName, propData]) => {
    ...
    const renderResourceType = (
      ...
      if (typeData.properties) {
        Object.entries(typeData.properties).forEach(
          ([fieldName, fieldData]) => {
    ...
    return (
      <Box key={ftsIndexData?.name || "no-index"} sx={{ p: 0 }}>
