...
def create_pgp_form_with_fake_data(client, patient_id: Any, encounter_id: Any, user_id: Any) -> Optional[Dict[str, Any]]:
  pid = str(patient_id)
  eid = str(encounter_id)
  prescriber_ref = {"reference": f"Practitioner/{user_id}"} if user_id else None

  frame_type = random.choice(["Full Rim", "Half Rim", "Rimless"])  # example values
  lens_type = random.choice(["Single Vision", "Bifocal", "Progressive"])  # example values
  condition_of_pg = random.choice(["Good", "Fair", "Poor"])  # example values
  duration_of_use = random.choice(["< 1 year", "1-2 years", "> 2 years"])  # example values
  pupil_distance = int(round(random.uniform(50.0, 70.0)))
  ...
  qr = {
    ...
    "authored": _now_iso(),
    "meta": {"tag": [{"system": FORM_TAG_SYSTEM, "code": FORM_CODE_PGP}]},
  }
  if prescriber_ref:
  ...
  vp_dv = {
    ...
    "note": [
      {"text": f"Frame: {frame_type}"},
      {"text": f"Lens: {lens_type}"},
      {"text": f"PD: {pupil_distance} mm"},
      {"text": f"Condition: {condition_of_pg}"},
      {"text": f"Duration: {duration_of_use}"},
    ],
    "meta": {"tag": [{"system": FORM_TAG_SYSTEM, "code": FORM_CODE_PGP}]},
  }
  if prescriber_ref:
  ...
  # List packaging as parent
  list_body = {
    "resourceType": "List",
    "status": "current",
    "mode": "working",
    "title": "PGP Form",
    "code": {"coding": [{"system": FORM_TAG_SYSTEM, "code": FORM_CODE_PGP}]},
    "subject": {"reference": f"Patient/{pid}"},
    "encounter": {"reference": f"Encounter/{eid}"},
    "date": _now_iso(),
    "meta": {"tag": [{"system": FORM_TAG_SYSTEM, "code": FORM_CODE_PGP}]},
